...
with st.sidebar:
    ...
    allow_hit = st.sidebar.checkbox("Allow -4 Hit (2 Transfers)", value=False)
...
def get_fpl_data(t_id, gw, horizon):
    ...
    try:
        ...
        return players[players["status"].isin(["a","d"])], owned_ids, bank
    except Exception as e:
        st.error(f"FPL Error: {e}")
        return None, [], 0.0

def run_optimizer(players, owned_ids, budget, is_wc, allow_hit):
    ...
    prob += pulp.lpSum([players.loc[i, 'xp'] * lineup[i] for i in players.index]) + \
            pulp.lpSum([loyalty for i in players.index if players.loc[i, 'id'] in owned_ids and s[i] == 1])
    ...
    for i in players.index: prob += lineup[i] <= s[i]  
    ...
    res = res.sort_values(by=['Status', 'element_type'], ascending=[True, True])
    starters = res[res['Status'] == "⚽ START"].sort_values(by='xp', ascending=False)
    return res, starters.iloc[0]['web_name'], starters.iloc[1]['web_name']

# --- MAIN APP ---
players, owned_ids, live_bank = get_fpl_data(team_id, current_gw, horizon)
    ...
    with tab1:
        ...
        with c1:
            if st.button("🚀 Optimize Wildcard"):
                sq, cap, vc = run_optimizer(players, owned_ids, initial_wealth - buffer, True, allow_hit)
                st.session_state.squad_ids = sq['id'].tolist()
                st.rerun()

        with c2:
            if st.button("🔄 Suggest Transfer Strategy"):
                sq, cap, vc = run_optimizer(players, owned_ids, initial_wealth - buffer, False, allow_hit)
                st.session_state.squad_ids = sq['id'].tolist()
                st.rerun()
